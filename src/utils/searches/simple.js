 ...
 */
export const searchSimple = ({ nodeI, nodeF, direction }) => {
  ...
  while (!nodeMeta) {
    ...
    if (successorsDistance.includes(0)) {
      ...
      const minDistance = successorsDistance.find(distance =>
        distance < DISTANCE[currentNode][nodeF]
      )
      ...
      if (!minDistance || minDistance >= DISTANCE[currentNode][nodeF]) {
        nodeMeta = successorsOfNode[successorsDistance.indexOf(
          Math.min(...successorsDistance)
        )]
        break
      }
